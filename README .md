...
1. The manuscript describes a strict patient-wise split using `lesion_id`, whereas the supplied training code currently calls `StratifiedKFold` directly on image rows.
2. The manuscript reports **124.3M** parameters, while the supplied training log reports **137.0M** parameters.
3. The code configuration uses an effective batch size of `8 × 4 = 32`, while the manuscript reports an effective batch size of 16.
4. The code configuration sets classification focal-loss `gamma=1.5`, while the code header describes `gamma=2`.
5. The supplied training log is an excerpt through epoch 21, while the manuscript reports final test-fold results after the full experimental workflow.
6. The manuscript states that patient-wise split indices were publicly provided, but those split-index files were not among the uploaded materials used to build this package.
